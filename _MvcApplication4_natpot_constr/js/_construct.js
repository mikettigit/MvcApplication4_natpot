...
$(document).ready(function(){
	...
	$('.cp_wall').farbtastic(function(color){
		...
		// //$('.wall').css('fill',color);
		// $('.color2').css('background-color',color);
		$('.svg_wall').css('background-color',color);
	...
	$('.but_send_construct').on('click',function(){
		var param_c = [];
		param_c['c_level_1']=$('.level_1').css('fill');
		param_c['c_level_2']=$('.level_2').css('fill');
		param_c['form']=$('.active_form').attr('value');
		param_c['material']=$('.active_m').attr('value');
		param_c['light'] = 0;
		if( $('.light').hasClass('show_light') ){
			param_c['light']=1;
		}

		param_c['light_l'] = 0;
		if( $('.light_l').hasClass('show_light') ){
			param_c['light_l']=1;
		}

		param_c['c_meb']=$('.color_meb').css('fill');
		param_c['c_wall']=$('.svg_wall').css('background-color');

		// param_c.each(function(i,val){
		// 	console.log(i+'='+val);
		// });

		var href_params = '?';
		for(var key in param_c){
			var hr = key+'='+param_c[key]+'&';
			href_params = href_params + hr;
		}

		// console.log(href_params);
		//$('.link_construct > a').attr('href',href_params);
		//$('.link_construct > a').text(href_params);
		//console.log(param_c);

		var form_data = go_data_fields($('.form_celling'));
		if( form_data == false ){
			return;
		}
		...
		$.post('to_mail.php',{
			...
			document.location.href="form-ok.html";
		});
